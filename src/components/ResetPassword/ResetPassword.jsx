...
export default function ResetPassword() {
  ...
  const initialValues = {  
   ...
   const validationSchema = Yup.object().shape({
    ...
    newPassword: Yup.string().min(6).max(15).required("New password is required").matches(/^[A-Z][a-zA-Z0-9]{5,14}$/,"New Password must be at least 6 characters"),
    });
   ...
   const formik = useFormik({
    ...
    return (
      <div className={`${classes.ResetPassword} max-h-screen py-[9.3rem]`}>
        ...
        <div className="container mx-auto">
          <h1 className='text-xl text-center font-bold mb-6 text-emerald-700'>Reset Password</h1>
      ...
      {successMsg && (<div className="alert alert-success max-w-lg mb-6 mx-auto text-center capitalize">{successMsg}</div>)}
        ...
        <form onSubmit={formik.handleSubmit} className='w-10/12 mx-auto'>
            ...
            <div className="relative z-0">
              <input type="password" id="newPassword" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-green-500 focus:outline-none focus:ring-0 focus:border-green-600 peer" placeholder=" " autoComplete='new-password'name='newPassword' value={formik.values.newPassword} onChange={formik.handleChange} onBlur={formik.handleBlur}/>

              <label htmlFor="newPassword" className="absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transhtmlForm -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 peer-focus:text-green-600 peer-focus:dark:text-green-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto">New Password</label>
